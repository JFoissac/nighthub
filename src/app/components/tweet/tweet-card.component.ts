  ...
  template: `
    @if (tweet()) {
      <a [href]="tweetUrl()" target="_blank" rel="noopener noreferrer"
         class="block hover:bg-white/[0.03] transition-colors cursor-pointer">
        <div class="flex gap-3 p-4 border-b border-[#1E1E2E]/50 last:border-0">
...
export class TweetCardComponent {
  ...
  tweetUrl(): string {
    const t = this.tweet();
    if (!t) return '#';
    const id = (t as any).twitterId || '';
    if (id.startsWith('http')) return id;
    const handle = (t.authorHandle || '').replace('@', '');
