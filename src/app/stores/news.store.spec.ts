import { TestBed, fakeAsync, tick } from '@angular/core/testing';
import { NewsStore } from './news.store';
import { ApiService } from '../services/api.service';
import { AiNewsItem } from '../models';
import { of, throwError } from 'rxjs';
import { patchState } from '@ngrx/signals';

const mockNews = (id: string, title: string): AiNewsItem => ({
  id,
  title,
  source: 'anthropic',
  url: `https://news.com/${id}`,
  summary: `Summary for ${title}`,
  timestamp: new Date(),
});

describe('NewsStore', () => {
  let store: any;
  let mockApi: any;

  beforeEach(() => {
    mockApi = {
      getVideos: jest.fn(),
      getNews: jest.fn(),
      getTweets: jest.fn(),
      getStreams: jest.fn(),
      getTrumpTweets: jest.fn(),
    };

    TestBed.configureTestingModule({
      providers: [
        { provide: ApiService, useValue: mockApi },
      ],
    });
    store = TestBed.inject(NewsStore) as any;
    patchState(store, { items: [], loading: false, atEnd: false, error: null });
  });

  describe('initial state', () => {
    it('should have empty items', () => {
      expect(store.items()).toEqual([]);
    });

    it('should not be loading', () => {
      expect(store.loading()).toBe(false);
    });

    it('should not be at end', () => {
      expect(store.atEnd()).toBe(false);
    });

    it('should have no error', () => {
      expect(store.error()).toBeNull();
    });
  });

  describe('computed signals', () => {
    it('news should return items', () => {
      expect(store.news()).toEqual([]);
    });

    it('isLoading should return loading state', () => {
      expect(store.isLoading()).toBe(false);
    });

    it('count should return items length', () => {
      patchState(store, { items: [mockNews('1', 'News 1')] });
      expect(store.count()).toBe(1);
    });

    it('hasError should return true when error exists', () => {
      patchState(store, { error: 'API error' });
      expect(store.hasError()).toBe(true);
    });
  });

  describe('loadMore', () => {
    it('should call apiService.getNews and update items on success', fakeAsync(() => {
      const items = [mockNews('1', 'News 1'), mockNews('2', 'News 2')];
      mockApi.getNews.mockReturnValue(of(items));
      store.loadMore();
      tick(50);
      expect(mockApi.getNews).toHaveBeenCalled();
      expect(store.items()).toEqual(items);
      expect(store.loading()).toBe(false);
    }));

    it('should set atEnd when response has fewer items', fakeAsync(() => {
      const items = [mockNews('1', 'News 1')];
      mockApi.getNews.mockReturnValue(of(items));
      store.loadMore();
      tick(50);
      expect(store.atEnd()).toBe(true);
    }));

    it('should not set atEnd when response is full', fakeAsync(() => {
      const items = Array(20).fill(null).map((_, i) => mockNews(String(i), `News ${i}`));
      mockApi.getNews.mockReturnValue(of(items));
      store.loadMore();
      tick(50);
      expect(store.atEnd()).toBe(false);
    }));

    it('should set error on API failure', fakeAsync(() => {
      mockApi.getNews.mockReturnValue(throwError(() => new Error('Network failure')));
      store.loadMore();
      tick(50);
      expect(store.error()).toContain('Network failure');
      expect(store.loading()).toBe(false);
    }));
  });

  describe('reload', () => {
    it('should reset state before reloading', fakeAsync(() => {
      const items = [mockNews('1', 'News 1')];
      mockApi.getNews.mockReturnValue(of(items));
      store.reload();
      tick(50);
      expect(store.items()).toEqual(items);
      expect(store.loading()).toBe(false);
    }));
  });

  describe('setItems', () => {
    it('should set items and reset atEnd', () => {
      patchState(store, { atEnd: true });
      const items = [mockNews('1', 'News 1')];
      store.setItems(items);
      expect(store.items()).toEqual(items);
      expect(store.atEnd()).toBe(false);
    });
  });

  describe('clearError', () => {
    it('should clear error', () => {
      patchState(store, { error: 'Error' });
      store.clearError();
      expect(store.error()).toBeNull();
    });
  });
});